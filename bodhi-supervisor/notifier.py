...
# win10toast (and the win32 APIs it pulls in) is Windows-only; guard the
# import so the rest of Bodhi can still run and be tested on other platforms.
try:
...
class Notifier:
    ...
    def available(self) -> bool:
        """Whether toast notifications are supported on this platform."""
    ...
    def notify(self, title: str, message: str, duration: int = 10,
        ...
        Returns:
            True if notification was shown, False if rate-limited or
            toasts are unavailable on this platform
        """
        if self.toaster is None:
            return False  # Toasts unsupported (non-Windows platform)
...
class BlockingIntervention:
    ...
    def show(self, title: str, message: str, require_response: bool = True) -> Optional[str]:
        ...
        if require_response:
            ttk.Label(
                main_frame, text="Explain your focus or justify this activity:", style="Big.TLabel"
        ...
        if require_response:
            ttk.Button(
                btn_frame, text="Submit & Continue", command=lambda: self._submit(dialog)
            ).pack(side=tk.RIGHT, padx=(10, 0))
            ttk.Button(
                btn_frame, text="I'm Back on Track", command=lambda: self._submit(dialog, "Returning to goal")
            ).pack(side=tk.RIGHT)
        else:
            ttk.Button(
                btn_frame, text="OK", command=lambda: self._submit(dialog, "Acknowledged")
            ).pack(side=tk.RIGHT)
