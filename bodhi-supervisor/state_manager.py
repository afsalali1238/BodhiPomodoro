...
from paths import STATE_FILE, state_lock, atomic_write_json
...
class StateManager:
    ...
    def _save(self) -> None:
        ...
        Runs under the shared ``state_lock`` and writes atomically so the
        monitor threads (which append activity via save_activity_entry) and
        this in-memory copy can never interleave into a torn write.
        """
        with state_lock:
