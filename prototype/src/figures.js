// Vector artwork with soft 3D shading: Bodhi tree, sitting Buddha, walking Buddha.
// Style reference: pale skin, orange robe over one shoulder, topknot, dark shades.
// Figures: origin = bottom-center (where he touches the ground).

const C = {
  skin: '#fbf3ea', skinMid: '#efdccb', skinDk: '#d9bfa8', line: '#6b4a3a',
  robe: '#f59a3c', robeMid: '#e67e22', robeDk: '#b85a14',
  hair: '#2b211d', hairHi: '#4a3b34', lens: '#141414'
};

// Gradients are injected once per page (ids are shared by both figures).
function defs() {
  return `
  <defs>
...
function head(hy) {
  ...
  <g class="head">
    ...
    <g fill="${C.hairHi}" opacity=".55">${[-9, -5, -1, 3, 7, -7, -3, 1, 5].map((x, i) =>
      `<circle cx="${x + (i > 4 ? 1 : 0)}" cy="${hy - (i > 4 ? 17 : 13.5)}" r="1.3"/>`).join('')}</g>
    ...
    <path d="M0,${hy + 3} q-1.2,3.2 0.2,4.6" stroke="${C.skinDk}" stroke-width=".9" fill="none" stroke-linecap="round"/>
...
// Raised hand (palm facing out, fingers up) centred at (x,y)
function raisedHand(x, y) {
  return `
  <g transform="translate(${x},${y})">
    ...
    <path d="M-4.2,-4 C-4.6,2 -3,5 0.5,5.5 C4,5.5 5.8,2 5.8,-4 Z" fill="url(#gSkin)" stroke="${C.skinDk}" stroke-width=".4"/>
...
function sittingBuddha() {
  return `
  <g class="body">
    <ellipse cx="0" cy="-1" rx="56" ry="8" fill="url(#gShadow)"/>
    <!-- crossed legs under robe -->
    ...
    <!-- torso: bare chest on the left, robe over the right shoulder -->
    ...
    <!-- left arm (robed) down to lap hand -->
    ...
    ${lapHand(2, -28)}
    <!-- meditation variant: right arm down to lap, hands stacked -->
    ...
    <!-- right arm (bare) raised, hand at chest -->
    <g class="arm-raised">
...
function blossoms(stage, rnd, blobs) {
  if (stage < 3) return '';
  const n = stage === 3 ? 7 : 16, out = [];
  for (let i = 0; i < n; i++) {
    const [cx, cy, rx, ry] = blobs[i % blobs.length];
    const a = rnd() * Math.PI * 2, r = Math.sqrt(rnd()) * .85;
    const x = cx + Math.cos(a) * rx * r, y = cy + Math.sin(a) * ry * r;
    out.push(`<g transform="translate(${x.toFixed(1)},${y.toFixed(1)})">${[0, 72, 144, 216, 288].map(d =>
      `<ellipse cx="0" cy="-2.2" rx="1.6" ry="2.4" fill="#ffe3ee" transform="rotate(${d})"/>`).join('')}<circle r="1.1" fill="#ffb74d"/></g>`);
  }
  return out.join('');
}

function leafPath() { return 'M0,-9 C-7,-9 -8,-1 -4,4 Q-1,7 0,12 Q1,7 4,4 C8,-1 7,-9 0,-9 Z'; }

function bodhiTree(stage = 4) {
  const density = [0.5, 0.64, 0.78, 0.9, 1][stage] || 1;
  let seed = 7;
  const rnd = () => (seed = (seed * 16807) % 2147483647) / 2147483647;
  const tones = ['#2e7d32', '#388e3c', '#43a047', '#1b5e20', '#4caf50', '#66bb6a'];
  const blobs = [[118, 92, 96, 62], [60, 118, 52, 36], [175, 110, 58, 40], [110, 52, 70, 38]];
  const leaves = [];
  for (const [cx, cy, rx, ry] of blobs) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const x = cx + Math.cos(a) * rx * r, y = cy + Math.sin(a) * ry * r;
      // leaves nearer the top-left catch the light
      const lit = ((cx + rx - x) / (2 * rx) + (cy + ry - y) / (2 * ry)) / 2 + rnd() * .35;
      const c = lit > .85 ? 'url(#gLeaf)' : tones[Math.min(tones.length - 1, Math.floor(rnd() * 4 + (1 - lit) * 2))];
      leaves.push(`<path d="${leafPath()}" fill="${c}" transform="translate(${x.toFixed(1)},${y.toFixed(1)}) rotate(${Math.round(rnd() * 360)}) scale(${(0.75 + rnd() * .6).toFixed(2)})"/>`);
    }
  }
  return `
  <g class="tree">
    ...
    <g class="canopy">${leaves.join('')}${blossoms(stage, rnd, blobs)}</g>
  </g>`;
}

const leafSvg = `<path d="${leafPath()}" fill="#7cb342" transform="scale(.8)"/>`;

window.BodhiArt = { defs, sittingBuddha, standingBuddha, bodhiTree, leafSvg, leafPath };
